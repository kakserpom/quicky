...
{
 static $obj = NULL;
 if ($obj === NULL) {$obj = new Quicky_joincalculator;}
 $params = $compiler->_parse_params($params);
 if (!isset($params['name'])) {return $compiler->_syntax_error('Missing parameter \'name\' in joincalculator tag.');}
 if (!isset($params['fields'])) {return $compiler->_syntax_error('Missing parameter \'fields\' in joincalculator tag.');}
...
';
  foreach ($params['fields'] as $k => $v)
  {
   ...
   {
    if (isset($fields[$q[2]])) {return $compiler->_syntax_error('Field name \''.$q[2].'\' alredy in use ');}
    $fields[$q[2]] = $q[1];
    $fields[$q[1]] = $q[1];
   }
   else {$fields[$v] = $v;}
  ...
  if ($f === '') {return $compiler->_syntax_error('No fields');}
  $obj->_tmp = $fields;
