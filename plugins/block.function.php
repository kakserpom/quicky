...
{
 $block_name = 'function';
 if (!preg_match('~^\s*(.*?)\s*\((.*)\)\s*$~',$params,$g)) {return $compiler->_syntax_error('Parse error in Function header.');}
 if (trim($g[2]) === '') {return $compiler->_syntax_error('Missing expression in Function tag.');}
 ...
 foreach ($args as $v)
 {
  if (!preg_match('~^\s*\$(\w+)\s*(?:=\s*(\-?\d+|NULL|([\'"]).*?(?<!\\\\)\3))?$~i',$v,$q)) {return $compiler->_syntax_error('Parse error in Function header.');}
  $args_names[] = $q[1];
