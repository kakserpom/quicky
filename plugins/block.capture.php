...
function quicky_block_capture($params,$content,$compiler)
{
 ...
 if (isset($params['ob'])) {$return = '<?php ob_start(); ?>'.$compiler->_tag_token($content,$block_name).'<?php $capture[\''.$name.'\'] = ob_get_contents(); ob_end_clean();'.($assign !== ''?' $var[\''.$assign.'\'] = $capture[\''.$name.'\'];':'').' ?>';}
 else
 {
